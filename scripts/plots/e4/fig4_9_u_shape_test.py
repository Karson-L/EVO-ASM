"""Fig 4.9: Diversity-Extreme U-shape.
Usage: python fig4_9.py --data-dir <path>
"""
import sys, argparse
...
def main(data_dir=None, output_dir=None):
    ...
    ax.scatter(entropy[:min_len], np.abs(returns[:min_len]), c="#3182BD", alpha=0.3, s=10)
    # Quadratic fit
    x,y = entropy[:min_len], np.abs(returns[:min_len])
    coeffs = np.polyfit(x, y, 2)
    xs = np.linspace(x.min(), x.max(), 100)
    ax.plot(xs, np.polyval(coeffs, xs), "r--", lw=2, label="Quadratic fit")
    ax.set_xlabel("Strategy Entropy");ax.set_ylabel("|Return|")
    ax.set_title("Figure 4.9: Diversity-Extreme Event Relationship")
    ax.legend()
    fig.savefig(output_dir/"fig4_9_u_shape_test.png");plt.close(fig)
    print("Saved Fig 4.9")
