...
Usage: python fig4_8.py --data-dir <path>
"""
import sys, argparse
...
def main(data_dir=None, output_dir=None):
    ...
    weights = agents[[f"weight_{i}" for i in range(6)]].values[:100]
    norms = np.linalg.norm(weights, axis=1, keepdims=True) + 1e-10
    w_norm = weights / norms
    sim = np.abs(w_norm @ w_norm.T)
    adj = (sim > 0.3).astype(float)
    fig,ax = plt.subplots(figsize=(10,8))
    ax.imshow(adj, cmap="Blues", aspect="auto")
    ax.set_xlabel("Agent");ax.set_ylabel("Agent")
    ax.set_title("Figure 4.8: Strategy Similarity Adjacency Matrix")
    fig.savefig(output_dir/"fig4_8_interaction_network.png");plt.close(fig)
    print("Saved Fig 4.8")
