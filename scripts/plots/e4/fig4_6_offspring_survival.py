"""Fig 4.6: Offspring survival rate.
Usage: python fig4_6.py --data-dir <path>
"""
import sys, argparse
...
def main(data_dir=None, output_dir=None):
    ...
    # Agents with parent_id != -1 are offspring
    if "parent_id" in agents.columns:
        offspring = agents[agents["parent_id"] != -1]
        total = len(agents); nos = len(offspring)
        fig,ax = plt.subplots(figsize=(6,6))
        ax.pie([total-nos, nos], labels=["Original","Offspring"], colors=["#3182BD","#E6550D"], autopct="%1.1f%%")
        ax.set_title("Figure 4.6: Offspring Survival Rate")
    fig.savefig(output_dir/"fig4_6_offspring_survival.png");plt.close(fig)
    print("Saved Fig 4.6")
