"""Fig 3.1: Strategy family decay.
Usage: python fig31.py --data-dir <path>
"""
import sys, argparse
...
def main(data_dir=None, output_dir=None):
    ...
    for i,sd in enumerate(subdirs[:3]):
        ...
        ax.plot(df["step"],df["n_families"],color=colors[i%3],lw=1.2,alpha=0.8,label=sd.name[:40])
    ax.set_xlabel("Step");ax.set_ylabel("Active Strategy Families")
    ax.set_title("Figure 3.1: Strategy Family Decay (Replication)")
    ax.legend(fontsize=8)
    fig.savefig(output_dir/"fig3_1_family_decay.png");plt.close(fig)
    print("Saved Fig 3.1")
