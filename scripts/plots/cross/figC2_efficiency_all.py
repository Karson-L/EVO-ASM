...
def main(data_dir=None, output_dir=None):
    ...
    for label, edir in EXPERIMENTS.items():
        ...
        if subdirs:
            ...
            ax.plot(df["step"],np.abs(df["acf1"]),color=colors[label],lw=1.5,label=label,alpha=0.8)
    ax.axhline(y=0,color="grey",lw=0.5)
    ax.set_xlabel("Step");ax.set_ylabel("|ACF(1)|")
    ax.set_title("Figure C.2: Market Efficiency Across All Experiments")
    ax.legend()
    fig.savefig(out_dir/"figC2_efficiency_all.png");plt.close(fig)
    print("Saved Fig C.2")
