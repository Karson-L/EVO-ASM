...
def main(data_dir=None, output_dir=None):
    ...
    ax.boxplot([alpha_data[k] for k in ["E1","E2","E3","E4"] if k in alpha_data],
               labels=[k for k in ["E1","E2","E3","E4"] if k in alpha_data])
    ax.set_ylabel("Return Distribution")
    ax.set_title("Figure C.4: Return Distribution Across Experiments")
    fig.savefig(out_dir/"figC4_halflife_all.png");plt.close(fig)
    print("Saved Fig C.4")
